...
//					$('.Ytype').find('.Yorder').removeClass('active');
		...
		//是否是编辑状态下的标题
		if(! $('.Ytype').hasClass('active')){
			$('.Ytype .Yorder.active + p').append(Ytxt);
		}else{
			$('.Ytype.active .Yorder.active+p').append(Ytxt);
		}
	...
	$('.Ymove').on('click','.Yanalysis_btn',function (){
		...
		if( ! $('.Yorder').hasClass('active')){
			...
			$('.Yorder.active').parent().siblings('.Yanalysis').find('p').append(Ytxt);
		}
		$('.Yorder').parent().siblings('.Yanswer').find('input').attr('disabled',true);
		$('.Yorder.active').parent().siblings('.Yanswer').find('input').attr('disabled',false);
	});
...
//						$('.Ymove').hide();
	...
	$('.Ytest_cases').on('dblclick','.Ytype',function (){
		...
		$(this).parents('.Ytest_cases').find('input').attr('disabled',true);
		...
		$(this).find('ul li').not('.Yanswer').children('p').each(function(){
			UE.getEditor($(this).siblings('.Yeditor').attr("id")).setContent($(this).html());
			$(this).siblings('.Yeditor').show();
			$(this).hide();
		});
	...
	$('.Ytest_cases').on('click','.Yedit_img',function (){
		...
		$(this).parents('.Ytype').find('ul li').not('.Yanswer').children('p').each(function(){
			UE.getEditor($(this).siblings('.Yeditor').attr("id")).setContent($(this).html());
			$(this).hide().siblings('.Yeditor').show();
		});
...
//		$(this).parents('.Ytype').find('ul li').not('.Yanswer').children('p').show();
		$('.Yeditor').hide();
		$(this).parents('.Ytype').find('ul li').not('.Yanswer').children('p').each(function(){
			console.log(UE.getEditor($(this).siblings('.Yeditor').attr("id")).getContent())
			$(this).html(UE.getEditor($(this).siblings('.Yeditor').attr("id")).getContent());
			$(this).show().siblings('.Yeditor').hide();
		});
	...
	$('.Ytest_cases').on('click','.Ydelete_img',function (){
		if($(this).parents('.Ytype').siblings('.Ytype').length == 0){
			$(this).parents('.Ytype').siblings('.Yheadline').hide();	//题型标题
			$(this).parents('.Yborder').hide();			//题型最下分割线
		}
		var YDobj = $(this).parents('.Ytype').parent();
		
		var YorderAct = 0;
		$('.Yorder').each(function (i,e){
		...
		$('.Ytype').each(function (i,ele){
			$(this).find('.Yorder').text(i + 1);	//题号
		});
		$('.Yorder').removeClass('active');
		
		if($('.Ytype').hasClass('active')){		//有正在编辑的试题
			$('.Ytype.active').find('.Yorder').addClass('active');		//题号当前背景色
		}else{
			if(YorderAct > $('.Yorder').length - 1) YorderAct = $('.Yorder').length - 1;
			$('.Yorder').eq(YorderAct).addClass('active');
		}
		...
		if($('.Ytype').length == 0){
			$('.Ysave').css('visibility','hidden');		//保存按钮
		}
		
		//销毁编辑器
		/*$(this).parents('.Ytype').find('.Yeditor').each(function (){
	...
	$('.Ytest_cases').on('click','.Ysave',function (){
		...
		$('.Ytype').parent().children('.Ytype').each(function (index,ele){
			...
			if($(this).find('.Yedit_over').css('display') == 'none'){
				$(this).find('.Yoption ol li').each(function (i,e){
					console.log($(e).find('p').text().trim())
					if($(e).children().text().trim() == ''){
						YemptyText = 2;
						return false;
					}else{
						YemptyText = 0;
		...
		if(YemptyText == 1){
			...
			//tzy
			$("#radiosum").html($(".Yradio .Ytype").length);
			$("#checksum").html($(".Ycheck .Ytype").length);
...
//				Ytotal += Number($(ele).val());
				Ytotal += Number($(ele).val())*$(ele).prev("span").html();
			}
		});
		$('.Ytotal_score>span').text(Ytotal + '分')
	});
